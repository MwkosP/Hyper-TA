...
def calculateEntropy(df, column='Close', bins=10,alpha=0.5):
...
def calculateRollingEntropy(df, window=40, column='Close', bins=10,alpha=0.5):
    """
    ...
    Parameters:
        ...
        column : price column
        bins : bin width (same as calculateEntropy)
    ...
    for i in range(window, len(df) + 1):
        ...
        normalized_entropy, std_bin_probability = calculateEntropy(window_df,column=column,bins=bins,alpha=alpha)
...
def visualizeEntropy(df, column='Close', bins=10):
